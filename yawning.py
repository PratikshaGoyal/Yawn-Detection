...
def yawnDetector(video_capture):
    ...
    # 		cv2.rectangle(frame, (int(widthOneCorner), int(heightOneCorner), (int(widthOtherCorner), int(heightOtherCorner),(0,0,255), 2)

        # mouth region
        mouthRegion = frame[int(heightOneCorner):int(heightOtherCorner), int(widthOneCorner):int(widthOtherCorner)]
...
def main():
    ...
    while True:
        ...
        if returnValue[0]:
            ...
            # testing 
            engine.say("I think you are feeling tired and sleepy") 
            engine.say("You can listen to some energetic songs to feel fresh") 
            engine.say("You can also open to window to get some fresh air") 
            engine.runAndWait() 
            # When everything is done, release the capture
#             if cv2.waitKey(1) & 0xFF == ord('q'):
#                 break
#             return returnValue
        elif returnValue[0] is None:
